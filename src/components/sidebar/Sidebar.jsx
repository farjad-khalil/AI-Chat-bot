...
function Sidebar() {
    ...
    return (
        <aside className={`flex h-screen flex-col justify-between border-r border-white/10 bg-slate-950/80 text-slate-100 backdrop-blur-xl ${state.extended ? 'w-72' : 'w-20'} transition-all duration-300 ease-in-out`}>
            ...
            <div className='flex items-center gap-3 px-4 pt-5'>
                <button type='button' onClick={toggle} className='inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 transition hover:bg-white/10'>
                    <img src={assets.menu_icon} alt='Toggle sidebar' className='w-[22px]' />
                </button>
                {state.extended ? <p className='text-sm font-medium tracking-[0.25em] text-cyan-300/80'>GEMINI</p> : null}
            </div>

            {/* New Chat Button */}
            <button type='button' className={`mx-4 mt-6 inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-cyan-400 px-4 py-3 text-slate-950 shadow-lg shadow-cyan-500/10 transition hover:bg-cyan-300 ${state.extended ? 'justify-start' : 'justify-center'}`}>
                <img src={assets.plus_icon} className='w-[18px]' alt='' />
                {state.extended ?
                ...
                {state.extended ? (
                    <div className='rounded-3xl border border-white/10 bg-white/5 p-4'>
                        ...
                        <div className='mt-4 flex items-center gap-3 rounded-2xl bg-white/5 px-3 py-3'>
                            <img src={assets.message_icon} className='w-[22px]' alt='' />
